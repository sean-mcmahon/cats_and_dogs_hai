...
class ClassificationInference:
    ...
    def predict(self, image_raw: torch.Tensor) -> Tuple[PetSpeciesPrediction, PetBreedPrediction]:
        image_processed = self.preprocess_transforms(image_raw).unsqueeze(0)
        predictions = self.model(image_processed)
        predictions_binary = (torch.nn.functional.sigmoid(predictions) >= self.threshold).type(torch.int)
